...
class ASPDNet(nn.Module):
    ...
    def forward(self, x):  # [16, 3, 256, 256]
        x = self.frontend(x)  # [16, 512, 32, 32]
        residual = x
        x = self.ca(x) * x  # [16, 512, 32, 32]
        x = self.sa(x) * x
        x += residual
        ...
        x2 = self.conv4_3_2(x)
        x3 = self.conv4_3_2(x)
        x4 = self.conv4_3_2(x)  # [16, 512, 32, 32]
        x = torch.cat((x1, x2, x3, x4), 1)  # [16, 2048, 32, 32]
        x = self.conv5(x)  # [16, 512, 32, 32]

        x = self.mid_end(x)

        offset1 = self.offset1(x)  # [16, 512, 32, 32]
        x = F.relu(self.conv6_1(x, offset1))  # [16, 256, 32, 32]
        x = self.bn6_1(x)

        offset2 = self.offset2(x)  # [16, 256, 32, 32]
        x = F.relu(self.conv6_2(x, offset2))  # [16, 128, 32, 32]
        x = self.bn6_2(x)

        offset3 = self.offset3(x)  # [16, 128, 32, 32]
        x = F.relu(self.conv6_3(x, offset3))  # [16, 64, 32, 32]
        x = self.bn6_3(x)  # [16, 64, 32, 32]

        x = self.output_layer(x)  # [16, 1, 32, 32]
        return x
...
class UncerNet(ASPDNet):
    ...
    def forward(self, x):
        ...
        if "after_vgg" in self.drop_stages:
            x = self.dropout(x)
        if "after_vgg" in self.ptb_stages:
            x = self.ptb(x)

        x = self.ca(x) * x
        if "after_ca" in self.drop_stages:
            x = self.dropout(x)
        if "after_ca" in self.ptb_stages:
            x = self.ptb(x)

        x = self.sa(x) * x
        if "after_sa" in self.drop_stages:
            x = self.dropout(x)
        if "after_sa" in self.ptb_stages:
            x = self.ptb(x)

        x += residual
        if "after_front" in self.drop_stages:
            x = self.dropout(x)
        if "after_front" in self.ptb_stages:
            x = self.ptb(x)
        ...
        if "inner_mid" in self.drop_stages:
            x = self.dropout(x)
        if "inner_mid" in self.ptb_stages:
            x = self.ptb(x)

        x = self.mid_end(x)
        if "after_mid" in self.drop_stages:
            x = self.dropout(x)
        if "after_mid" in self.ptb_stages:
            x = self.ptb(x)
        ...
        if "after_offset1" in self.drop_stages:
            x = self.dropout(x)
        if "after_offset1" in self.ptb_stages:
            x = self.ptb(x)
        ...
        if "after_offset2" in self.drop_stages:
            x = self.dropout(x)
        if "after_offset2" in self.ptb_stages:
            x = self.ptb(x)
        ...
        if "after_offset3" in self.drop_stages:
            x = self.dropout(x)
        if "after_offset3" in self.ptb_stages:
            x = self.ptb(x)

        x = self.output_layer(x)
        if "after_all" in self.drop_stages:
            x = self.dropout(x)
        if "after_all" in self.ptb_stages:
            x = self.ptb(x)
